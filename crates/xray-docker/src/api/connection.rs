...
impl DockerApiConnection {
    ...
    pub fn connect<R: AsRef<str>>(pipe: R) -> Result<Self> {
        let pipe_stream =
            DuplexPipeStream::<pipe_mode::Bytes>::connect_by_path(
                // Strip the named pipe addr prefix if it's present
                pipe.as_ref()
                    .strip_prefix(r"npipe://")
                    .unwrap_or(pipe.as_ref()),
            )
            .map_err(|e| {
                DockerError::from_io_error_with_description(e, || {
                    "failed to connect to the Docker named pipe".into()
                })
            })?;
    ...
    pub fn make_request(&mut self, buf: &mut Vec<u8>) -> Result<StatusCode> {
        ...
        let (response_code, parsing_mode) =
            self.read_response_meta(buf, &mut temp_buf)?;
    ...
    fn read_response_meta(
        ...
        buf: &mut Vec<u8>,
        temp_buf: &mut [u8],
    ) -> Result<(StatusCode, BodyParsingMode)> {
        buf.clear();
        let socket = self.get_socket();
        ...
        loop {
            let filled_bytes = socket.read(temp_buf).map_err(|e| {
                DockerError::from_io_error_with_description(e, || {
                    "failed to read an HTTP response".into()
                })
            })?;
            ...
            let Ok(status) = response.parse(buf) else {
                return Err(DockerError::Other(
                    "failed to parse an HTTP response".into(),
                ));
            };
            ...
            let response_code =
                StatusCode::from_u16(response.code.ok_or_else(|| {
                    DockerError::Other(
                        "parsed an HTTP response without a code".into(),
                    )
                })?)
                .map_err(|_| {
                    DockerError::Other(
                        "got an invalid HTTP response code from Docker API"
                            .into(),
                    )
                })?;
            ...
            let body_bytes =
                &temp_buf[filled_bytes - read_body_bytes..filled_bytes];
            ...
            let parsing_mode = match body_type.name {
                "Content-Length" => {
                    ...
                    let content_length =
                        raw_content_length.parse::<usize>().map_err(|_| {
                            DockerError::Other(
                                "failed to parse content-length as a number"
                                    .into(),
                            )
                        })?;
                ...
                _ => {
                    return Err(DockerError::Other(
                        "found an unexpected header".into(),
                    ));
                }
            };
    ...
    fn read_response_body(
        ...
        loop {
            ...
            match &mut parsing_mode {
                BodyParsingMode::Chunks(chunk_processor) => {
                    if !chunk_processor.process_available_data(
                        &temp_buf[..filled_bytes],
                        buf,
                    )? {
                        break;
                    }
    ...
    fn get_socket(&mut self) -> &mut dyn ReadWrite {
        match self {
            #[cfg(unix)]
            DockerApiConnection::Unix(unix_sock) => {
                unix_sock as &mut dyn ReadWrite
            }
            #[cfg(windows)]
